...
const resetBtn = document.getElementById('reset-btnn');
  ...
  if (guess === word) {
    ...
    if (attempts === 3) {
      ...
      info.innerHTML = alert(
        'Sorry. You are out of chances. Click on OK to restart.'
      );
      reset();
      setLevel();
    }
...
function setLevel() {
  if (level == 1) {
    ...
    info.innerHTML = alert(
      'You Win! Great job!. You can reset or keep playing.'
    );
  }
...
window.addEventListener(
  'keypress',
  function (e) {
    if (e.code == 'Enter' && userGuess.value.length > 1) {
      checkAnswer(userGuess.value.toLowerCase());
      userGuess.value = '';
    }
  },
